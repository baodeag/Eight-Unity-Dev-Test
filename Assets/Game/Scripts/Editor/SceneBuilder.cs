...
namespace baodeag.Game.Editor
{
    public static class SceneBuilder
    {
        private const string ScenePath = "Assets/Game/Scenes/Landscape.unity";
        private const string GemTypeFolder = "Assets/Game/Data/GemTypes";
        private const string GemMaterialFolder = "Assets/Game/Materials/Gems";
        private const string AnimatorPath = "Assets/Game/Animations/Player.controller";
        private const string PlayerPrefabPath = "Assets/Game/Prefabs/Characters/Player.prefab";
        private const string GemPrefabPath = "Assets/Game/Prefabs/Collectibles/Gem.prefab";
        private const string GemVisualPrefabPath = "Assets/Game/Prefabs/Collectibles/Visual/Gem Visual.prefab";
        private const string ConfettiPrefabPath = "Assets/Game/Prefabs/VFX/ConfettiBlastRainbow.prefab";
        ...
        public static void BuildScene()
        {
            ...
            GemType commonType = CreateGemType("Common Gem", 1, 70, commonMaterial, new Color(0.2f, 0.95f, 1f));
            GemType rareType = CreateGemType("Rare Gem", 2, 25, rareMaterial, new Color(0.45f, 1f, 0.3f));
            GemType epicType = CreateGemType("Epic Gem", 3, 5, epicMaterial, new Color(1f, 0.35f, 0.85f));

            RuntimeAnimatorController animatorController = CreateAnimatorController();
            Gem gemPrefab = CreateGemPrefab(commonMaterial);
            ...
            IntroCameraSequence intro = introObject.AddComponent<IntroCameraSequence>();
            ...
            BuildManagers(player.GetComponent<PlayerController>(), cameraController, intro, spawner, uiManager, player.transform);

            EditorSceneManager.SaveScene(scene, ScenePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            UnityEngine.Debug.Log($"Game scene built at {ScenePath}");
        }
        ...
        private static void EnsureFolders()
        {
            ...
            EnsureFolder("Assets/Game/Data", "GemTypes");
            EnsureFolder("Assets/Game", "Materials");
            EnsureFolder("Assets/Game/Materials", "Gems");
            EnsureFolder("Assets/Game", "Animations");
            EnsureFolder("Assets/Game", "Prefabs");
            EnsureFolder("Assets/Game/Prefabs", "Characters");
            EnsureFolder("Assets/Game/Prefabs", "Collectibles");
            EnsureFolder("Assets/Game/Prefabs/Collectibles", "Visual");
            EnsureFolder("Assets/Game/Prefabs", "VFX");
        ...
        {
            string path = $"{GemMaterialFolder}/{name}.mat";
        ...
        private static GemType CreateGemType(string name, int score, int weight, Material material, Color lightColor)
        {
            string path = $"{GemTypeFolder}/{name}.asset";
            GemType gemType = AssetDatabase.LoadAssetAtPath<GemType>(path);
            if (gemType == null)
            {
                gemType = ScriptableObject.CreateInstance<GemType>();
        ...
        private static Gem CreateGemPrefab(Material material)
        {
            GameObject root = new GameObject("Gem");
            root.layer = LayerMask.NameToLayer("Gem");

            GameObject sourceGem = AssetDatabase.LoadAssetAtPath<GameObject>(GemVisualPrefabPath);
        ...
        private static GameObject CreatePlayerPrefab(RuntimeAnimatorController animatorController)
        {
            ...
            ClimbDetector climb = root.AddComponent<ClimbDetector>();
        ...
        private static Boundary BuildBoundary()
        {
            GameObject boundaryObject = new GameObject("Map Boundary");
            Boundary boundary = boundaryObject.AddComponent<Boundary>();
        ...
        private static void BuildGemSystem(Gem gemPrefab, Transform player, GemType common, GemType rare, GemType epic, out GemSpawner spawner)
        {
            GameObject system = new GameObject("Gem System");
            GemFactory factory = system.AddComponent<GemFactory>();
            GemPool pool = system.AddComponent<GemPool>();
            spawner = system.AddComponent<GemSpawner>();
        ...
        private static void BuildAttackButton(Transform parent, PlayerController player)
        {
            Button button = BuildButton("Attack Button", parent, "ATK", new Vector2(1f, 0f), new Vector2(150f, 150f), new Vector2(-160f, 145f));
            AttackButton attackButton = button.gameObject.AddComponent<AttackButton>();
        ...
        private static void WirePlayer(GameObject player, VirtualJoystick joystick, CameraController cameraController, Boundary boundary)
        {
        ...
        private static void BuildManagers(PlayerController player, CameraController camera, IntroCameraSequence intro, GemSpawner spawner, UIManager ui, Transform playerTransform)
        {
            ...
            GameObject confettiPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(ConfettiPrefabPath);
